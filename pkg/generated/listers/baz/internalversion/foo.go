...
type FooLister interface {
	...
	List(selector labels.Selector) (ret []*baz.Foo, err error)
...
// List lists all Foos in the indexer.
func (s *fooLister) List(selector labels.Selector) (ret []*baz.Foo, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*baz.Foo))
	})
	return ret, err
}
...
// List lists all Foos in the indexer for a given namespace.
func (s fooNamespaceLister) List(selector labels.Selector) (ret []*baz.Foo, err error) {
...
// Get retrieves the Foo from the indexer for a given namespace and name.
func (s fooNamespaceLister) Get(name string) (*baz.Foo, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(baz.Resource("foo"), name)
	}
	return obj.(*baz.Foo), nil
}
